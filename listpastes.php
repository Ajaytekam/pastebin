...
if(isset($_SESSION["LoggedIn"]) && $_SESSION["LoggedIn"] === true){
    $sql = "select p_id, pu_id, title, lang,encrypt_status, creation_time, deletion_time from reg_pastes union select p_id, null, title, lang,encrypt_status, creation_time, deletion_time from unreg_pastes order by creation_time desc"; 
    ...
    $sql = "select p_id, pu_id, title, lang,encrypt_status, creation_time, deletion_time from reg_pastes where pasteExposure = 'public' union select p_id, null, title, lang,encrypt_status, creation_time, deletion_time from unreg_pastes order by creation_time desc"; 
    $result = mysqli_query($conn, $sql);
    $row = mysqli_fetch_all($result, MYSQLI_ASSOC);
    $rowcount = mysqli_num_rows($result);
    for($i = 0; $i < $rowcount; $i++) {
